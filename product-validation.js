...
(() => {
  ...
  function normalizeProduct(input,defaults){
    ...
    if(d.practices===undefined) d.practices={items:[],completions:{}};
    const practices=d.practices;
    if(!object(practices)||!Array.isArray(practices.items)||!object(practices.completions))fail('kişisel pratikler');
    const practiceIds=new Set();
    for(const item of practices.items){if(!object(item)||!id(item.id)||practiceIds.has(item.id)||!string(item.name,100)||!item.name.trim()||!Array.isArray(item.days)||!item.days.length||new Set(item.days).size!==item.days.length||item.days.some(n=>!Number.isInteger(n)||n<0||n>6)||!date(item.createdAt)||typeof item.archived!=='boolean'||(item.templateKey!=null&&!string(item.templateKey,40)))fail('pratik');practiceIds.add(item.id)}
    if(Object.entries(practices.completions).some(([day,entries])=>!date(day)||!object(entries)||Object.entries(entries).some(([key,value])=>!practiceIds.has(key)||typeof value!=='boolean')))fail('pratik işaretleri');
    if(d.notifications!==undefined){const n=d.notifications;if(!object(n)||['focus','rest','events'].some(k=>typeof n[k]!=='boolean')||!object(n.daily)||Object.entries(n.daily).some(([k,t])=>!['planner','tasks','routine','sport','calorie','practices','journal','budget','invest'].includes(k)||typeof t!=='string'||!/^([01]\d|2[0-3]):[0-5]\d$/.test(t)))fail('bildirim ayarları')}
